...
| `includes/top-bar.php` | The site's top bar on a phone, on every page (1.17.0): `js/rm-top-bar.js` slides the element that pins the theme's header away while the visitor scrolls down past 120 px and brings it back on the first scroll up, as droneracingslovakia.com does - only while the navigation shows its burger, never with its menu open. Shown, the bar must have no transform: one would make it the frame of the burger's fixed overlay. |
| `includes/race-info.php` | What a race tells a pilot outside its pages (1.19.0): its location (`_race_location`, the meta box), its calendar entry (`/?rm_race_calendar=<ID>`, UTC, out of page caches) and the registration mail's tags - `[_race_title]`, `[_race_dates]`, the links - for the race the CF7 form's `race_id` names. `[rm_nextup]` is read from page text, never with `has_shortcode()`, which knows it only on live pages. |
...
| `includes/pilot-key.php` | The pilot key: one version-5 UUID per registered email address, the same every time, derived in a namespace of this site's own. What RotorHazard is to match a returning pilot by, since `user_id` is 0 for everyone without an account. `rm_registration_rows()` in `admin-registrations.php` puts it into the admin list, the CSV and `get-pilots` alike. One address is one pilot, so one registration per address and race (1.19.1): the CF7 form refuses a second one (`rm_validate_one_registration_per_pilot()`, message `rm_already_registered` in the form's Messages tab), the admin page too, and `[rm_registered]` lists a pilot registered twice before once - addresses compared as the key reads them, `rm_pilot_address()`. |
...
| `npm run test:pilot-profiles` | the registration's confirmation mail from Mailpit: the race by title, dates, place, links, the calendar entry to download; nationality and photo sent through the real registration form: the country stored, the photo square, 256 px, upright and without metadata with GD and with Imagick, carried in a race's files, gone with the registration. Makes and removes its race, form and page through WP-CLI |
...
| `npm run test:top-bar` | the site's top bar on a phone: gone while scrolling down past 120 px, back on the first scroll up, never with the burger's menu open, only while the burger shows and the header is pinned. Browser only, no WordPress |
