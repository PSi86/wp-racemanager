...
| `js/rm-m-updateStatus.js` | The freshness line above every live view — the only consumer of `onState()`. Its `describe()` is pure and exported so the state machine can be tested without a broken network. |
