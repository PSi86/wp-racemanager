...
| `pilot-key` | The pilot key: a version-5 UUID computed as RFC 9562 defines it (checked against Python's documented `uuid5` value), one key per address whatever its case or blanks and the same on every call, derived in the site's own namespace — created once, another one giving other keys — and `RM_PILOT_NAMESPACE` winning, an invalid one handing out no key rather than falling back (in a PHP process of its own, since a constant cannot be undefined). And that every row of `get-pilots` carries the key, checked on the endpoint itself; against the old endpoint that check fails. |
