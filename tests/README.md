...
| `activation` | What the activation hook leaves behind, and what a *second* activation must not: the `CREATE TABLE` statement `dbDelta()` can actually parse, with the subscriptions' `pilot_key`, the CF7 example form being created once rather than once per activation, and the plugin header — one version number in three places, plus the `Requires` headers. And that an update brings the table up to date without the hook, which a ZIP replace does not run: on `plugins_loaded`, `dbDelta()` once, the schema recorded only once the column is there, and not again after; and a subscription stored with its pilot key, or none. Schema 3 (1.13.0) adds `channel`, NULL by default, is recorded only once that column is there, and a site on schema 2 runs `dbDelta()` once more; against 1.12.2 those four checks fail. |
| `push-channel` | What the "your next race" push says about the channel (1.13.0), which `rm_getUpcomingRacePilots()` names only once a heat's seats are fixed. A heat from the generator before the race director calls it: the schedule without a channel, and then the channel, told as given now - also on seat 0, the number an unknown seat is stored under - and nothing on the next upload. The channel told is kept in `channel` and compared as text: the same channel is no news, another seat or the same seat on another frequency is a change; the plan reset is told; another heat without a channel yet says nothing about one, with the same or another channel "remains" or "is". A subscription stored before 1.13.0 (`channel` NULL) is compared by heat and slot as 1.12 did, so an update does not repeat the pushes of a race under way. Until the column is there nothing is written to it. Against 1.12.2 nine checks fail; those of the subscriptions from before pass there, as they should. And the likely channel (1.15.0): with the schedule, marked ("Channel likely R3") and kept as `R3?`; nothing on the next upload; called on it, nothing said and `R3` stored; called on another, the change; another one likely, or one likely only from a later upload on, told; one no longer to be told, nothing said and nothing written, so that `R3?` stands; the plan reset with a likely channel said so; another heat with its likely channel; a fixed channel before a likely one. Against 1.14.0 eleven of these fail; the other four hold there too. |
| `subscription-switch` | A browser's one subscription moved to another pilot or race (1.13.1), through the real `rm_upsert_subscription()`, `rm_get_subscriptions()` and push handler on one row kept in memory. Moved to another pilot, the schedule starts over: nothing is said while the new pilot is not up - 1.13.0 told the follower they had been removed from the heat of the pilot before - and then "Next race is ...", also in the heat of the pilot before, with or without a channel; moved to another race likewise. The same pilot of the same race keeps it: subscribed again by ID, found by the key under a new ID after the timer re-created its pilots, a subscription from before keys now given one; the same ID with another key is someone else and starts over. Before the channel column exists, the schedule starts over without writing it. Against 1.13.0's `db-handler.php` seven checks fail; those of the same pilot pass there, as they should. |
...
| `race-winner` | Who won a race (1.12.0). The final of every plan RotorHazard ships (`tests/fixtures/brackets/plans.json`): a single elimination's Final and not its Small Final, which no heat seeds from either, in whatever order the timer lists them; the double eliminations' Final, MultiGP's "Race 14: Winners Bracket Final", a ladder's A Main; none for a ranked fill or a class whose heats do not seed each other. The winner: the first of the final's result with the pilot key the timer sent, nobody before the final has a result or when its first has no place; a class ranked with "Brackets" by its place 1, whatever the final says; Chase the Ace undecided or without a ranking, nobody; switched off, the final decides; one winner per bracket class, in the timer's order. Kept in `_race_winner`, the page cache emptied when it changes and not otherwise; the races stored before worked out from their data files, a batch per admin request, then no more. The block: callsign escaped, photo with its version over the initials, flag, the class only when there are several, nothing without a winner or on a post that is no race. On the real events of the local site, one winner each, the first of the final. Four mutations of the rules each fail a check. The podium (1.14.0): the final's first three in its order, a pilot without a place included, a final of two with two; the "Brackets" ranking's places 1 to 3, a shared one too; none without a winner; kept with the schema it was worked out with, and a site on schema 1 working its races out again. The block's setting: unset the winner as before; the podium a medal a place with the place said for screen readers, a podium per bracket class under its name; a race not worked out again its winner; the standing on the race list's cards the podium with nothing loaded, on the race's own page the podium and under it the standing's container, with the standing's module and stylesheet and not the bracket's, configured with its container and the flags; another race's page the podium; a setting that is none the winner. On the real events, each podium the final's first three. Against 1.13.1, 22 of the 67 checks fail. And the link to the results (1.18.0), to the race's bracket view: none unless set; a line "Results" under the podium, or under the standing on the race's page; the whole podium or winner as the link, with "Results:" for a screen reader and the standing outside the link; no link for a setting that is none, nor on a site without a live area. Against 1.17.0 the five link checks fail. |
...
| `race-info` | What a race tells a pilot outside its pages (1.19.0): the tags of the registration mail name the race it was sent for - title as typed, dates as the race-date block shows them over one day and over two, start and end, the location as typed and on a map, its page, its live area, its next-up view found by its `[rm_nextup]` (read from the page's text: `has_shortcode()` answers false where the shortcode is not registered, as it is not where a mail goes out, and the stub answers so), its calendar entry; empty where the race has nothing to say, for a draft's calendar, a race without dates, a race field naming a page, no race chosen, outside a submission; CF7's own tags and another filter's answer left alone; in an HTML mail the text escaped, line breaks kept. The calendar entry: one event, 09:00 in Vienna 07:00Z in June and 08:00Z in December, title and location escaped, CRLF lines, a long title folded at 75 octets without splitting a character; none for a draft, a race without dates or a page. The location trimmed, an empty field keeping nothing. The Mail tab lists the tags for a form with a race field only. |
| `registration-once` | One registration per pilot and race (1.19.1). An address is registered for a race when one of its registrations has it, whatever its case and blanks, as the pilot key reads addresses; another race's does not count, no address is nobody. The form refuses a second one at the address field with the form's own message, which its Messages tab lists and can change; another address, another race, a race field naming no race, a form without a race field or without the address field, an address refused already and a request outside a submission are left alone - the validation run as CF7 runs it, through the callbacks registered for `wpcf7_validate` with their number of arguments. The race page lists a pilot registered twice before once, at the first registration's place under the callsign sent last; registrations without an address are one pilot each. A registration added by hand is refused likewise, another race's not in the way. Against 1.19.0 ten of the 23 fail, the guards passing there too. |
| `nextup-schedule` | Who flies next, as every upload works it out for the "your next race" pushes (`rm_getUpcomingRacePilots()`). The heat on the timer is announced when it is the last one, and when the timer numbered its heats from 50 — the loop's guard counted from the heat's id and gave up at once in both cases, so no next-up push went out for a final. A slot the timer fills from a class's result (method 2) takes that class's pilot, from its ranking when it has one, not the pilot of the heat that carries the class's number; a slot filled from a heat's result takes the entry at `seed_rank - 1`, as RotorHazard seeds. Against 1.9.0 all five checks fail; with the guard fixed alone, the three seed checks still do, each for its own reason. And a Chase the Ace final (1.10.0) - the class ranked with "Brackets", its switch untouched - stays the heat to come until the timer's ranking has its place 1, whatever the class's number of rounds says; decided, or switched off, it is done after its round. Against 1.9.1 the undecided case fails. And the seat and its channel (1.13.0): the slot's node, not its place in the list - which differ while RotorHazard gives out a heat's seats, the free slots then without node and first - and named only once the seats are fixed: by hand, confirmed or flown, not for a generated heat before it is called; a frequency no band names as the frequency, a node switched off with none. Against 1.12.2 all six fail. And until then the channel the pilot will likely get (1.15.0), as RotorHazard's adaptive assignment gives it without drawing lots: each from a seat of their own before, that seat; the plan confirmed, the channel and nothing likely; two from one seat, none for them and the third still told; a seat only one of them flew on is theirs first; the seats flown on by start time, not by where the rounds are listed, and a seat flown on again once, as the last; none for a pilot who has not flown or whose seat is switched off; none for anyone while a seed's heat or class has no result, but a seed beyond its result brings nobody and the others are told. Against 1.14.0 all twelve fail. And where the upload carries RotorHazard's own `used_frequencies` (1.16.0), those count, matched by frequency: the frequency RotorHazard has for a pilot, not the seat of the rounds; none for a frequency the profile lacks, nor for an empty list whatever the rounds say; a frequency only one of them flew is theirs first. Against 1.15.0 all four fail. |
...
null (RotorHazard 4.4.0) or 0 (before), or a class the timer no longer has - get a section of their own, "Unclassified Heats", last; an
event without classes shows its heats there, and the section goes when its heats get a class (1.12.2;
1.12.1 drew them nowhere - four of the six checks failed against it). A heat shows a line per pilot
and per slot its seeding will fill, none for a free seat, on an eight-node timer's heats of eight slots
(1.13.0): each line with its node's channel ("R1", its frequency as the tooltip) once the seats are
fixed - by hand, confirmed or flown - and none for a heat from the generator before it is called;
the frequency where no band names it, "–" for a node switched off, no channel without frequency
data; the channel its own element before the callsign, which stays by it in a line with room to
spare, and pointing at it marks the pilot. Against 1.12.2 the line checks fail - eight lines a heat -
and the layout check fails with 1.12.2's stylesheet (the callsign 48 px off the channel). Before the
seats are fixed, the channel each pilot will likely get (1.15.0), from the seats flown on before:
"R1?", marked and held back, the frequency with "fixed when the heat is called" as its tooltip; an
empty place for a pilot who has not flown, so that the callsigns stand in line; no channel for
...
from a real browser - and since 1.19.0 the confirmation mail that registration sends, read from DDEV's
Mailpit: the race by its title, in the subject too, not by its ID; its dates; where it takes place and
on a map; its page, its calendar entry and its next-up view; no tag left unfilled; and the calendar
entry downloaded - `text/calendar`, the race, its place, in UTC. Against 1.18.0 five of those seven
fail; the first run found the next-up link on the bracket view, since `has_shortcode()` does not know
`[rm_nextup]` outside the live pages. And since 1.19.1 the same address sent again for the race, in
capitals and without a photo: refused at the address field, saying why; no second mail - told apart
from the first by Mailpit's message IDs, since the second submission follows the first mail within a
second, too close for a time to tell; the registration stored once. Against 1.19.0 those three fail,
and the delete with them, which then deletes two.

A real submission it has to be: Contact Form 7 deletes its uploads once a submission is done, so only
a real submission shows that the photo is taken in time. `tests/e2e/pilot-profiles-site.php`, through
...
showed "3/2"; found on the RotorHazard 4.4.0 bench with that plugin). And the seat a pilot will likely
get (1.15.0), the same cases as `nextup-schedule` has for the push: a seat of their own before; two
from one seat left to chance, the third still told; a seat only one of them flew on theirs first, as
RotorHazard's adaptive assignment fills the seats; the seats flown on by start time, each once; none
for a pilot who has not flown or whose seat is switched off. All eight fail against 1.14.0. By
RotorHazard's own `used_frequencies` where the data has them (1.16.0), the four cases of
`nextup-schedule`; all four fail against 1.15.0. And the
three real events of the local site, when there: FAI 32 double elimination each, laid out without
overlap; on race 32, flown with automatic frequencies, each heat's likely seats from the rounds before
it against the seats flown (81 of 82 on 2026-09-13; the check wants 95 %).
...
And places 1 to 3 as the race's podium keeps them (1.14.0): `includes/race-winner.php` stores, without
the timer's ranking, the final's first three in its result's order, and the race's cards show that
while its page shows this standing. For every bracket plan RotorHazard ships, full and with 12
pilots, and on the three real events, the standing's first three are those.
...
| `race-winner`'s setting (1.14.0) | The sidebar offers winner, podium and standing; chosen, the podium shows as such in the canvas and is saved as `{"show":"podium"}` and nothing else - the block being dynamic, anything more in its markup would be content an update could invalidate. Its link to the results (1.18.0): no link, a line, the whole block; chosen, the line shows as "Results →" under it and is saved as `"link":"line"`. Against 1.17.0 those two checks fail. |
