...
| `compressed-bodies` | A timer's gzip-compressed body on the `rm/v1` routes: decoded on `rest_pre_dispatch`, before core would refuse it as invalid JSON, whatever the header's case, `x-gzip` too, and a 1.9 MB event as well as a small one; left alone without the header, for `identity`, on another namespace and when another filter has answered; refused with 400 when it is no gzip or cut short, with 415 for another encoding, and with 400 when it inflates past 10 MB — a 19 kB body that inflates to 20 MB stops there, the memory it took measured, because `gzdecode()`'s own limit does not hold; not inflated at all for a stranger, who gets the gate's 401 or 403 first. And that every answer of the namespace, a 415 included, carries `Accept-Encoding: gzip`, and no other does. |
