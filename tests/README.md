...
| `race-winner`'s setting (1.14.0) | The sidebar offers winner, podium and standing; chosen, the podium shows as such in the canvas and is saved as `{"show":"podium"}` and nothing else - the block being dynamic, anything more in its markup would be content an update could invalidate. |
